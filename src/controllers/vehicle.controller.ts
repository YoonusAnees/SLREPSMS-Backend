...
import { z } from "zod";
import { addVehicle, listMyVehicles ,verifyVehicleOwnership } from "../services/vehicle.service.js";
...
export async function createVehicle(req: Request, res: Response) {
 ...
 const dto = z.object({
  ...
  insuranceExpiry: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(), // YYYY-MM-DD
}).parse(req.body);
...
export async function verifyVehicle(req: Request, res: Response) {
  ...
  const raw = (req.params as any).plateNo as unknown;
