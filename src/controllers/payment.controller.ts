...
export async function stripeCreateCheckout(req: Request, res: Response) {
  const userId = (req as any).user.sub;
  ...
  const result = await createStripeCheckout(userId, dto);

  res.json({
    paymentId: result.payment.id,
    checkoutUrl: result.checkoutUrl,
    amountLkr: result.payment.amountLkr,
    receiptNo: result.payment.receiptNo,
  });
}

export async function stripeCheckoutSuccess(req: Request, res: Response) {
  const dto = z.object({
    paymentId: z.string().uuid(),
  }).parse(req.body);

  const payment = await confirmStripeCheckoutSuccess(dto.paymentId);
  res.json(payment);
}
